...
namespace Mind {
    ...
    Rectangle2Df::Rectangle2Df() : m_isCentered(false), m_hasChanged(false) {}

    Rectangle2Df::Rectangle2Df(const Rectangle2Df& copied) : m_isCentered(copied.m_isCentered),
                                                             m_hasChanged(true),
    ...
    Rectangle2Df::Rectangle2Df(
        ...
        if (centered) {
            Point2Df relativePosition = Point2Df(
                                                 m_size.getWidth() / 2.f,
                                                 m_size.getHeight() / 2.f
                                                ) ;
            m_bottomLeftCorner = Point2Df(
    ...
    Rectangle2Df::Rectangle2Df(
                               ...
                               const Point2Df& p2
                                  ...
                                  m_hasChanged(false) {
        std::pair<Scalar, Scalar> minMax_X = Math::minmax(p1.getX(), p2.getX()) ;
        std::pair<Scalar, Scalar> minMax_Y = Math::minmax(p1.getY(), p2.getY()) ;
    ...
    Rectangle2Df::Rectangle2Df(
                               ...
                               const Scalar& y2
                                  ...
                                  m_hasChanged(false) {
        std::pair<Scalar, Scalar> minMax_X = Math::minmax(x1, x2) ;
        std::pair<Scalar, Scalar> minMax_Y = Math::minmax(y1, y2) ;
    ...
    bool Rectangle2Df::intersects(const Line2Df& line) {
        Scalar x = Math::min(line.getStartX(), line.getEndX()) ;
        Scalar y = Math::min(line.getStartY(), line.getEndY()) ;
        Scalar w = Math::abs(line.getEndX() - line.getStartX()) ;
        Scalar h = Math::abs(line.getEndY() - line.getStartY()) ;
    ...
    Rectangle2Df operator+(Rectangle2Df& r, Point2Df& p) {
        if (r.contains(p)) {
            ...
            Scalar x1 = Math::min(r.getBottomLeftCornerX(), p.getX()) ;
            Scalar y1 = Math::min(r.getBottomLeftCornerY(), p.getY()) ;
            Scalar x2 = Math::max(r.getUpRightCornerX(), p.getX()) ;
            Scalar y2 = Math::max(r.getUpRightCornerY(), p.getY()) ;
    ...
    Rectangle2Df operator+(Rectangle2Df& r1, Rectangle2Df& r2) {
        if (r1.contains(r2)) {
            ...
            Scalar x1 = Math::min(r1.getBottomLeftCornerX(), r2.getBottomLeftCornerX()) ;
            Scalar y1 = Math::min(r1.getBottomLeftCornerY(), r2.getBottomLeftCornerY()) ;
            Scalar x2 = Math::max(r1.getUpRightCornerX(), r2.getUpRightCornerX()) ;
            Scalar y2 = Math::max(r1.getUpRightCornerY(), r2.getUpRightCornerY()) ;
            return Rectangle2Df(x1, y1, x2, y2) ;
        }
    }
