...
static void itest_allocation() {
    Line2Di l1 ;
    assert(l1.getStartX() == 0) ;
    assert(l1.getStartY() == 0) ;
    assert(l1.getEndX() == 0) ;
    assert(l1.getEndY() == 0) ;
...
static void itest_utilities() {
    ...
    assert(expectedL1SquaredLength == expectedL1SquaredLength) ;
    double expectedL1Length = 467 ;
    double computedL1Length = sqrt(computedL1SquaredLength) ;
    assert(expectedL1Length == computedL1Length) ;

    assert(l1.length() == computedL1Length) ;
    assert(l1.lengthSquared() == computedL1SquaredLength) ;
    ...
    // Relative CCW
    assert(Line2Di::CCW(l1, Point2Di(0, 500)) == 1) ;
    assert(Line2Di::CCW(l1, Point2Di(0, 0)) == -1) ;

    // Relative line to plan origin
    Line2Di relativeL1 = Line2Di::relative(l1) ;
    assert(relativeL1.getStartX() == 0) ;
    assert(relativeL1.getStartY() == 0) ;
...
static void itest_intersection() {
    // Line 1 (horizontal)
    int p1StartX = -50 ;
    int p1StartY = 10 ;
    Point2Di p1Start(p1StartX, p1StartY) ;
