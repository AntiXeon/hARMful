...
const int AmountModulesToCopy = 2 ;
const std::array<std::string, AmountModulesToCopy> ModulesToCopy = { "amount_lights", "block_bindings" } ;
...
void includeAsShaderModule() {
    for (int moduleIndex = 0 ; moduleIndex < AmountModulesToCopy ; ++moduleIndex) {
		std::string moduleToCopy = ModulesToCopy[moduleIndex] ;
		std::string pathInclude = ShadersDirectory + PathSeparator + moduleToCopy + IncludeExtension ;
		...
		for (auto& p: fs::recursive_directory_iterator(ShadersDirectory)) {
		    if (fs::is_directory(p)) {
		        ...
		        if (p.path().filename().string() == WantedShaderFolderCopy) {
		            ...
		            output_file << WarningAutomaticGeneration << std::endl ;
		            output_file << getFileContent(pathInclude, false) ;
		            output_file.close() ;
		        }
		    }
		}
